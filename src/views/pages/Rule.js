...
const CardInfo = [
   {
      ...
      detail: [
         ...
         {
            main: "7시에 출석체크",
            sub: "시스템 문제나 정당한 개인 사유로 인해 지각할 시, 팀원들과 임원들에게 사전 연락 필수",
         },
         {
            main: "출석체크와 함께 공모전 진행 상황 + 피드백에 대한 피드백 + 공지사항",
            sub: "피드백은 [공모자들] 홈페이지의 피드백에 게시해주세요.",
         },
         {
            main: "팀장을 정해 서기 작업",
            sub: "회의록 양식은 제공 예정",
   ...
   },
   {
      ...
      detail: [
         ...
         {
            main: "공모전 수상 시, 상금의 10%는 동아리의 운영 및 활동 비용으로 기여되고, 활용될 예정입니다.",
         },
      ],
   },
   {
   ...
   },
   {
      ...
      detail: [
         ...
         {
            main: "공모전 팀 채팅방",
            sub: "팀마다 Supporter 1명이 팀 가이드를 위해 채팅방에 함께 참여합니다.",
         },
         // {
         //   main: '익명보장 오픈 채팅방',
         //   sub:
...
class Rule extends React.Component {
   ...
   render() {
      return (
         <div>
            <DemoNavbar />
            <Background lottieName={lottieRule} title="운영 방안과 규칙" desc="공모자들, 이것만은 꼭! 지켜주세요." />
                                 ...
                                 <Col lg="3" key={i}>
                                    <Card className="card-lift--hover shadow border-0">
                                       <CardBody className="py-5">
                                          ...
                                          <h6 className={contact.h6property}>{contact.title}</h6>
                                          <p className="description mt-3">{contact.text}</p>
                                          <div>
                                             {contact.badgeInfo.map((badgeContact, i) => {
                                                return (
                                                   <Badge color={badgeContact.color} pill className="mr-1" key={i}>
                                                      {badgeContact.text}
                                                   </Badge>
                                                );
                                             })}
